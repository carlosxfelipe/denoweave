...
try {
  ...
  const payloadPath = new URL('./input.txt', import.meta.url);

  const script = Deno.readTextFileSync(scriptPath);
  const rawText = Deno.readTextFileSync(payloadPath);

  // Payload is a raw string — the DWL script handles parsing
  const payload = parse(rawText, 'text');
