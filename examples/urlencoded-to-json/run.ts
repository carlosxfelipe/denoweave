...
try {
  ...
  const payloadPath = new URL('./input.urlencoded', import.meta.url);

  const script = Deno.readTextFileSync(scriptPath);
  const rawForm = Deno.readTextFileSync(payloadPath);

  // Parse application/x-www-form-urlencoded — uses native URLSearchParams
  const payload = parse(rawForm, 'urlencoded');
