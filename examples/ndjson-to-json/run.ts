...
try {
  ...
  const payloadPath = new URL('./input.ndjson', import.meta.url);

  const script = Deno.readTextFileSync(scriptPath);
  const rawNdjson = Deno.readTextFileSync(payloadPath);

  // Parse each NDJSON line into an array of objects
  const payload = parse(rawNdjson, 'ndjson');
