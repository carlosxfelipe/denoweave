...
import { parseCSV, toCSV, type CsvOptions } from './csv.ts';
...
export function parse(input: string, format: Format, options?: CsvOptions): Value {
  switch (format) {
    case 'json': return parseJSON(input);
    case 'csv':  return parseCSV(input, options);
...
export function serialize(value: Value, format: Format, options?: { indent?: number } & CsvOptions): string {
  switch (format) {
    case 'json': return toJSON(value, options?.indent ?? 2);
    case 'csv':  return toCSV(value, options);
    case 'xml':  return toXML(value, options?.indent ?? 2);
