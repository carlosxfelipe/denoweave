...
export const MATH_FUNCTIONS: Record<string, Value> = {
  ...
  pow:    ((base: Value, exp: Value): Value => Math.pow(base as number, exp as number)) as DWFunction,
  log:    ((n: Value): Value => Math.log(n as number)) as DWFunction,
  log10:  ((n: Value): Value => Math.log10(n as number)) as DWFunction,
  mod:    ((a: Value, b: Value): Value => (a as number) % (b as number)) as DWFunction,
  random: ((): Value => Math.random()) as DWFunction,
  PI:     Math.PI,
  E:      Math.E,

  // Scalar min/max (vs. array min/max in array.ts)
  minOf:  ((a: Value, b: Value): Value => Math.min(a as number, b as number)) as DWFunction,
  maxOf:  ((a: Value, b: Value): Value => Math.max(a as number, b as number)) as DWFunction,

  // Type helpers
  isNull:    ((v: Value): Value => v === null) as DWFunction,
  isEmpty:   ((v: Value): Value => {
    if (v === null || v === undefined) return true;
    if (typeof v === 'string' || Array.isArray(v)) return (v as string | Value[]).length === 0;
  ...
  length: ((x: Value): Value => {
    if (typeof x === 'string' || Array.isArray(x)) return (x as string | Value[]).length;
